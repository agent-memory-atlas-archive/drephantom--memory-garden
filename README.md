...
以下是实际运行界面的截图，全部使用[虚构示例笔记](examples/showcase-vault)，不包含个人笔记、聊天或本机路径。截图展示界面功能，不代表真实用户的认知变化或模型效果。
...
![合成示例：全局关系图与两条原文并排阅读](docs/images/graph.png)

想亲自试试，可以在“连接笔记库”中输入本机 `examples/showcase-vault` 文件夹的完整路径。这个示例包含 9 篇虚构笔记，无需导入私人资料。
