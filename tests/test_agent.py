    ...
    class FakeCloudClient:
        ...
        def chat_with_tools(self, messages, tools, timeout_seconds=None, tool_choice=None):
            ...
            if tools:
                ...
                source = database.fetchone('SELECT text FROM source_atoms WHERE id=?', (atom_id,))
                assert source is not None
                return {
                    "content": "这段工具过程不能呈现给用户。",
                    "tool_calls": [{'function': {'name': 'finish_response', 'arguments':
                        json.dumps({'conclusion': {'answer_type': 'insufficient_evidence',
                            'summary': f'这条记录可以作为本轮核对起点 [A{atom_id}]，但证据仍有限。',
                            'sources': [{'atom_id': atom_id, 'quote': source['text'][:80]}]}})}}],
                }
            return {
