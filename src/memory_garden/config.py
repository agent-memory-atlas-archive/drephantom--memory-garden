...
class Settings:
    ...
    workspace_registry_path: Path | None = None  # Local UI selection; separate from model settings.
    ...
    llm_max_input_chars: int = 60000  # 完整 chat 请求的字符预算；不是特定模型的精确 token 数。
    ...
    def load(cls, project_root: Path | None = None) -> Settings:
        ...
        settings = cls(
            ...
            workspace_registry_path=Path(env['MG_WORKSPACE_REGISTRY_PATH']) if env.get('MG_WORKSPACE_REGISTRY_PATH') else None,
            ...
            llm_max_input_chars=int(env.get("MG_LLM_MAX_INPUT_CHARS", "60000")),
