...
CREATE TABLE IF NOT EXISTS memory_items (
    ...
    source_verdict_id INTEGER NOT NULL UNIQUE REFERENCES verdicts(id),
    evidence_json TEXT NOT NULL DEFAULT '{}',
    created_at TEXT NOT NULL,
    updated_at TEXT NOT NULL
);
CREATE INDEX IF NOT EXISTS idx_memory_topic ON memory_items(topic_key, status, id);

-- 请求先持久化，再运行 Agent；页面离开不会丢失请求身份。
...
CREATE INDEX IF NOT EXISTS idx_jobs_thread ON ask_jobs(thread_id, created_at);
"""
    ...
    def initialize(self) -> None:
        ...
        with connection:
            ...
            if current < 5:
                ...
                if 'reply_to_user_message_id' not in message_columns:
                    connection.execute('ALTER TABLE messages ADD COLUMN reply_to_user_message_id INTEGER REFERENCES messages(id)')
                columns = {r[1] for r in connection.execute('PRAGMA table_info(verdicts)')}
                for name, definition in {
                    ...
                    if name not in columns:
                        connection.execute(f'ALTER TABLE verdicts ADD COLUMN {name} {definition}')
            if current < 6:
                source_columns = {r[1] for r in connection.execute('PRAGMA table_info(sources)')}
                atom_columns = {r[1] for r in connection.execute('PRAGMA table_info(source_atoms)')}
                if 'source_kind' not in source_columns:
                    connection.execute("ALTER TABLE sources ADD COLUMN source_kind TEXT NOT NULL DEFAULT 'vault'")
                if 'sender' not in atom_columns:
                    connection.execute("ALTER TABLE source_atoms ADD COLUMN sender TEXT NOT NULL DEFAULT ''")
            connection.execute(
        ...
        if current < 5:
            ...
            MemoryService(self).backfill()
        # Mark the migration complete only after provenance/memory backfill.
        # A failed backfill leaves the old version and can safely resume.
        self.execute(
            "INSERT INTO schema_meta(key, value) VALUES('schema_version', ?) "
            "ON CONFLICT(key) DO UPDATE SET value=excluded.value", (str(SCHEMA_VERSION),),
        )
        self.execute("INSERT OR IGNORE INTO schema_meta(key,value) VALUES('workspace_id',?)", (uuid.uuid4().hex,))
