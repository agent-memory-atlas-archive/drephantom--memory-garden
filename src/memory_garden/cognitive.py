...
from .memory import MemoryService, answer_scope, topic_key_for
...
class VerdictService:
    ...
    def save_verdict(
        ...
        *,
        ...
        topic, answer = answer_scope(self.database, message_id)
        if topic_key is None:
            topic_key = topic_key_for(topic)
        if not confirmed_interpretation:
            confirmed_interpretation = str(answer.get('summary') or '')
        now = utc_now()
        with self.database.transaction() as connection:
            connection.execute("UPDATE verdicts SET status='superseded' WHERE message_id=? AND status='active'", (message_id,))
            connection.execute("UPDATE memory_items SET status='superseded',updated_at=? WHERE source_message_id=? AND status='active'", (now, message_id))
            cursor = connection.execute(
                'INSERT INTO verdicts(message_id,topic_key,verdict,user_revision,confirmed_interpretation,'
                'missing_event,accepted_atom_ids_json,rejected_atom_ids_json,created_at,topic_label) '
                'VALUES(?,?,?,?,?,?,?,?,?,?)',
                (message_id, topic_key, verdict, user_revision.strip() or None,
                 ...
                 json.dumps(sorted(set(rejected_atom_ids or []))), now, topic),
            )
            verdict_id = int(cursor.lastrowid or 0)
            MemoryService(self.database).record_verdict(verdict_id)
    ...
    def for_topic(self, topic_key: str, limit: int = 6) -> list[dict[str, Any]]:
        rows = self.database.fetchall(
            "SELECT * FROM verdicts WHERE topic_key=? AND status='active' "
            "AND (message_id IS NULL OR id=(SELECT MAX(v.id) FROM verdicts v WHERE v.message_id=verdicts.message_id)) "
            "ORDER BY id DESC LIMIT ?", (topic_key_for(topic_key), min(max(limit, 1), 20))
        )
        unavailable = {
            item['source_verdict_id'] for item in MemoryService(self.database).list_items(topic_key, limit=100)
            if not item['available_for_recall']
        }
        return [dict(row) for row in rows if row['id'] not in unavailable and row['verdict'] != 'defer']
...
class DiscoveryService:
    ...
    def review(
        ...
        with self.database.transaction() as connection:
            connection.execute(
                "UPDATE discoveries SET status='reviewed', review_verdict=?, review_revision=? WHERE id=?",
                (verdict, user_revision.strip() or None, discovery_id),
            )
            connection.execute(
                "UPDATE verdicts SET status='superseded' WHERE source_discovery_id=? AND status='active'", (discovery_id,),
            )
            connection.execute(
                "UPDATE memory_items SET status='superseded',updated_at=? WHERE source_verdict_id IN "
                "(SELECT id FROM verdicts WHERE source_discovery_id=?) AND status='active'", (utc_now(), discovery_id),
            )
            cursor = connection.execute(
                'INSERT INTO verdicts(message_id,topic_key,verdict,user_revision,missing_event,'
                'accepted_atom_ids_json,rejected_atom_ids_json,created_at,source_discovery_id) '
                "VALUES(NULL,?,?,?,?, '[]','[]',?,?)",
                (str(row['topic_key']), verdict, user_revision.strip() or None,
                 missing_event.strip() or None, utc_now(), discovery_id),
            )
            verdict_id = int(cursor.lastrowid or 0)
            MemoryService(self.database).record_verdict(verdict_id)
