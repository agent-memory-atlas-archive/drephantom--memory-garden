...
from .context_budget import render_observation
...
class ToolObservation(BaseModel):
    ...
    def render(self) -> str:
        return render_observation(self.tool, self.data, self.boundary)
...
class CognitiveTools:
    ...
    def get_user_verdicts(self, args: dict[str, Any]) -> ToolObservation:
        ...
        rows = VerdictService(self.database).for_topic(topic_key)
        return ToolObservation(
            ...
            boundary="用户判定针对当时这份解释及其证据；尊重修正和否认，不扩大成整个主题永久不变。已撤回或被新判定替代的记忆不参与读取。",
        )
