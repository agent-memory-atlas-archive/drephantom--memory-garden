...
async function initialize(){
  ...
  try{
    ...
    if(active.public_demo_mode){const note=document.getElementById('demo-note');note.hidden=false;note.textContent=active.demo_use_model?'这是 Agent 演示。对话和合成笔记片段会发送到已配置的生成模型；私人笔记库及旧对话不在此演示中。':'这是离线演示，使用模板对照。要体验模型理解与工具调用，可运行 start-agent-demo.bat。';field('vault_path').readOnly=true;}
